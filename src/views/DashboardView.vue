...
function applyPlanToGrid(plan: MealPlan | null): void {
  ...
  for (const dayObj of plan.weeklyPlan) {
    ...
    mealData.value[key] = {
      Breakfast: {
        name: dayObj.breakfast?.name ?? '',
        calories: dayObj.breakfast?.calories,
        prepTime: dayObj.breakfast?.prepTimeMinutes,
      },
      Lunch: {
        name: dayObj.lunch?.name ?? '',
        calories: dayObj.lunch?.calories,
        prepTime: dayObj.lunch?.prepTimeMinutes,
      },
      Dinner: {
        name: dayObj.dinner?.name ?? '',
        calories: dayObj.dinner?.calories,
        prepTime: dayObj.dinner?.prepTimeMinutes,
      },
    }
  }
                ...
                >
                  <div class="py-2.5 px-2 flex flex-col items-center relative">
                    ...
                    <template v-if="isPast(getWeekDate(i))">
                      ...
                      <span
                        class="font-spaceGrotesk font-black text-slate-900/60 uppercase text-sm tracking-tight leading-none mt-1 line-through decoration-2"
                        >{{ day }}</span
                      >
                      <span
                        class="font-tomorrow text-[10px] text-slate-600/70 font-bold tracking-wide mt-0.5"
                        >{{ formatDate(getWeekDate(i)) }}</span
                      >
                    </template>
                    ...
                    <template v-else-if="isToday(getWeekDate(i))">
                      ...
                      >
                      <span
                        class="font-tomorrow text-[10px] text-slate-800 font-black tracking-wide mt-0.5 bg-slate-900 text-stone-50 px-2 py-0.5 rounded-full"
                        >TODAY</span
                      >
                    </template>
              ...
              <tr v-for="(meal, mealIdx) in MEALS" :key="meal" class="meal-row">
                ...
                <td
                  ...
                  :class="[
                    ...
                    mealIdx === MEALS.length - 1 && dayIdx === DAYS.length - 1
                      ? 'rounded-br-2xl'
                      : '',
