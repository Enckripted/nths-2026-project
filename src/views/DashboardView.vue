...
import MasterChef from '@/components/Prompt.vue'
...
function formatDate(d: Date): string { return `${MONTH_SHORT[d.getMonth()]} ${d.getDate()}` }
function dayLabel(d: Date): string   { return DAY_SHORT[d.getDay()] as string }
function isToday(d: Date): boolean   { return d.getTime() === today.getTime() }

const dayColorsByDow: Record<number, string> = {
  0: 'bg-teal-300',   1: 'bg-sky-300',
  2: 'bg-rose-300',   3: 'bg-emerald-300',
  4: 'bg-amber-300',  5: 'bg-violet-300',
  6: 'bg-pink-300',
}
function dayColor(d: Date): string { return dayColorsByDow[d.getDay()] ?? 'bg-slate-300' }

const MEALS = ['Breakfast', 'Lunch', 'Dinner'] as const

// ── State ─────────────────────────────────────────────────────────────────
const showIngredients = ref(false)
const showChef        = ref(false)
...
function getMeal(dayIdx: number, meal: string) {
  ...
  return dayPlan[meal.toLowerCase() as 'breakfast' | 'lunch' | 'dinner']
}
  ...
  <div class="dashboard-page min-h-screen bg-stone-50 flex flex-col">
    ...
    <div class="calendar-section flex-1 flex flex-col">
      <div class="section-label-bar bg-stone-100 border-b-2 border-slate-900 px-8 lg:px-14 py-3 flex items-center gap-4">
        ...
        <span v-else            class="font-tomorrow text-xs font-bold text-slate-400 uppercase tracking-widest">
          ...
          <table class="w-full h-full border-collapse min-w-[700px]" style="table-layout: fixed">
                ...
                >
                  <div class="py-2.5 px-2 flex flex-col items-center">
                    <template v-if="isToday(date)">
                      <span class="font-spaceGrotesk font-black text-slate-900 uppercase text-base tracking-tight leading-none">{{ dayLabel(date) }}</span>
                      <span class="font-tomorrow text-[10px] font-black tracking-wide mt-0.5 bg-slate-900 text-stone-50 px-2 py-0.5 rounded-full">TODAY</span>
                    </template>
                    <template v-else>
                      <span class="font-spaceGrotesk font-black text-slate-900 uppercase text-base tracking-tight leading-none">{{ dayLabel(date) }}</span>
                      <span class="font-tomorrow text-[10px] text-slate-700 font-bold tracking-wide mt-0.5">{{ formatDate(date) }}</span>
                    </template>
                  </div>
                </th>
              </tr>
            ...
            <tbody>
              <tr v-for="(meal, mealIdx) in MEALS" :key="meal" class="meal-row">
                <td :class="['row-label border-r-4 border-slate-900 text-center', mealIdx === MEALS.length - 1 ? 'rounded-bl-2xl border-b-0' : 'border-b-2 border-slate-200']">
                ...
                <td
                  v-for="(date, dayIdx) in weekDates" :key="dayIdx + meal"
                  :class="[
                    ...
                    mealIdx === MEALS.length - 1 && dayIdx === weekDates.length - 1 ? 'rounded-br-2xl' : '',
                  ]"
                >
                  <div v-if="loading" class="h-full min-h-[120px] p-3 flex flex-col gap-2 animate-pulse">
                  ...
                  <div v-else-if="!planGenerated || !getMeal(dayIdx, meal)?.name" class="empty-cell h-full min-h-[120px] flex flex-col items-center justify-center gap-2 opacity-35">
                  ...
                  >
                    ...
                    <div class="flex flex-wrap gap-1 text-[9px] font-tomorrow font-bold">
                      <span v-if="getMeal(dayIdx, meal)?.protein" class="text-violet-700 bg-violet-100 border border-violet-300 px-1.5 py-0.5 rounded-full">{{ getMeal(dayIdx, meal)?.protein }}g P</span>
                      <span v-if="getMeal(dayIdx, meal)?.carbs"   class="text-sky-700 bg-sky-100 border border-sky-300 px-1.5 py-0.5 rounded-full">{{ getMeal(dayIdx, meal)?.carbs }}g C</span>
                      <span v-if="getMeal(dayIdx, meal)?.fat"     class="text-rose-700 bg-rose-100 border border-rose-300 px-1.5 py-0.5 rounded-full">{{ getMeal(dayIdx, meal)?.fat }}g F</span>
                    </div>
                    <div class="mt-auto flex flex-wrap gap-1">
                      <span v-if="getMeal(dayIdx, meal)?.calories"        class="font-tomorrow text-[9px] font-bold text-emerald-700 bg-emerald-100 border border-emerald-300 px-1.5 py-0.5 rounded-full">{{ getMeal(dayIdx, meal)?.calories }} kcal</span>
                      <span v-if="getMeal(dayIdx, meal)?.prepTimeMinutes" class="font-tomorrow text-[9px] font-bold text-amber-700 bg-amber-100 border border-amber-300 px-1.5 py-0.5 rounded-full">{{ getMeal(dayIdx, meal)?.prepTimeMinutes }}min</span>
                    </div>
                  </div>
                </td>
              </tr>
    ...
    <!-- ══════════════════════════════════════════════════════
         MASTER CHEF — SLIDE-UP PANEL
    ══════════════════════════════════════════════════════ -->
    <Transition name="chef-panel">
      <div v-if="showChef" class="chef-panel fixed bottom-0 left-0 right-0 z-40 max-h-[70vh] overflow-y-auto">
        <MasterChef />
      </div>
    </Transition>
    ...
    ══════════════════════════════════════════════════════ -->
    <button
      @click="showChef = !showChef"
      :class="[
        'chef-fab fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-5 py-3',
        'font-spaceGrotesk font-black text-sm uppercase tracking-widest',
        'border-2 border-slate-900 rounded-2xl transition-all duration-200',
        'shadow-[4px_4px_0px_rgba(15,23,42,1)] hover:shadow-[2px_2px_0px_rgba(15,23,42,1)] hover:translate-x-[2px] hover:translate-y-[2px]',
        showChef
          ? 'bg-slate-900 text-amber-300'
          : 'bg-amber-300 text-slate-900 hover:bg-amber-200',
      ]"
    >
      <span class="text-base leading-none">{{ showChef ? '✕' : '👨‍🍳' }}</span>
      <span>{{ showChef ? 'Close Chef' : 'Master Chef' }}</span>
    </button>
  </div>
</template>
...
/* ── Chef panel slide-up transition ── */
.chef-panel-enter-active,
.chef-panel-leave-active {
  transition: transform 0.35s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.25s ease;
}
.chef-panel-enter-from,
.chef-panel-leave-to {
...
.chef-panel {
  box-shadow: 0 -6px 40px rgba(15, 23, 42, 0.5);
}
...
table             { height: 100%; background: #eff9e5; }
.row-label-header { background: #eff9e5; }
